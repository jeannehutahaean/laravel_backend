...
@section('title', 'Tambah Rute Pengiriman')

@section('content')
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Tambah Rute Pengiriman</h1>
    ...
    <form method="POST" action="{{ route('admin.routes.store') }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="location_name" required class="w-full border p-2 rounded" placeholder="Contoh: Toko Harun Salatiga">
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="bg-blue-50 p-3 rounded mb-2">
                <p class="text-sm text-blue-700">
                    <strong>Petunjuk:</strong> Klik pada peta untuk menentukan titik awal (merah) dan titik tujuan (biru). 
                    Klik pertama = titik awal, klik kedua = titik tujuan. Untuk mengubah titik, drag marker yang sudah ada.
                </p>
            </div>
            <div class="relative">
            ...
            <!-- Info Rute -->
            <div id="route_info" class="mt-3 p-3 bg-gray-50 rounded hidden">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <strong>Jarak:</strong> <span id="distance_display">-</span>
                    </div>
                    <div>
                        <strong>Estimasi Waktu:</strong> <span id="duration_display">-</span>
                    </div>
                </div>
            </div>
        ...
        <!-- Tombol Submit -->
        <button type="submit" id="submit_btn" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 disabled:bg-gray-400" disabled>
            Simpan Rute
        </button>
        <div id="submit_message" class="text-sm text-gray-500 mt-2">
            Silakan tentukan titik awal dan tujuan untuk melanjutkan
        </div>
    </form>
</div>
...
<script>
    ...
    const defaultCoords = [-7.005145, 110.438125]; // Yogyakarta

    // Inisialisasi peta
    const routeMap = L.map('route_map').setView(defaultCoords, 13);
    ...
    async function calculateRoute() {
        ...
        try {
            ...
            document.getElementById('submit_message').textContent = 'Rute siap disimpan';
            document.getElementById('submit_message').className = 'text-sm text-green-600 mt-2';
    ...
    document.getElementById('reset_route_btn').addEventListener('click', function(e) {
        ...
        document.getElementById('submit_message').textContent = 'Silakan tentukan titik awal dan tujuan untuk melanjutkan';
        document.getElementById('submit_message').className = 'text-sm text-gray-500 mt-2';
    });
